...
const DefaultNamespace = "qitmeer"
...
func toUsage(r ChatResponse) Usage {
	return Usage{
...
func toToolCalls(tc []ReqToolCall) []ToolCall {
...
func toChatCompletion(id string, r ChatResponse) ChatCompletion {
...
func toChunk(id string, r ChatResponse, toolCallSent bool) ChatCompletionChunk {
	toolCalls := toToolCalls(r.Message.ToolCalls)
...
func toUsageGenerate(r GenerateResponse) Usage {
	return Usage{
		PromptTokens:     r.PromptEvalCount,
		CompletionTokens: r.EvalCount,
		TotalTokens:      r.PromptEvalCount + r.EvalCount,
...
func toCompletion(id string, r GenerateResponse) Completion {
	return Completion{
		Id:                id,
...
func toCompleteChunk(id string, r GenerateResponse) CompletionChunk {
...
func toListCompletion(r ListResponse) ListCompletion {
	var data []Model
...
func toEmbeddingList(model string, r EmbedResponse) EmbeddingList {
	if r.Embeddings != nil {
		var data []Embedding
...
func toModel(r ShowResponse, m string) Model {
	return Model{
		Id:      m,
		Object:  "model",
...
func fromChatRequest(r ChatCompletionRequest) (*ChatRequest, error) {
	var messages []ReqMessage
	for _, msg := range r.Messages {
		switch content := msg.Content.(type) {
		case string:
			messages = append(messages, ReqMessage{Role: msg.Role, Content: content})
		case []any:
			for _, c := range content {
				...
				case "text":
					...
					messages = append(messages, ReqMessage{Role: msg.Role, Content: text})
				case "image_url":
					var url string
					...
					messages = append(messages, ReqMessage{Role: msg.Role, Images: []ImageData{img}})
				default:
					return nil, errors.New("invalid message format")
				}
			}
		default:
			...
			toolCalls := make([]ReqToolCall, len(msg.ToolCalls))
			...
			messages = append(messages, ReqMessage{Role: msg.Role, ToolCalls: toolCalls})
		}
	}
...
func fromCompleteRequest(r CompletionRequest) (GenerateRequest, error) {
	options := make(map[string]any)
	...
	case []any:
		...
		for _, s := range stop {
			if str, ok := s.(string); ok {
				...
				return GenerateRequest{}, fmt.Errorf("invalid type for 'stop' field: %T", s)
			}
		}
		options["stop"] = stops
	}
...
func RetrieveMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		var b bytes.Buffer
		if err := json.NewEncoder(&b).Encode(ShowRequest{Name: c.Param("model")}); err != nil {
...
func EmbeddingsMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var b bytes.Buffer
		if err := json.NewEncoder(&b).Encode(EmbedRequest{Model: req.Model, Input: req.Input}); err != nil {
			c.AbortWithStatusJSON(http.StatusInternalServerError, NewError(http.StatusInternalServerError, err.Error()))
			return
		}
